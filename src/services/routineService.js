...
export const generateVaccinationSchedule = async (chatId, dateOfBirth) => {
  ...
  await VaccineSchedule.deleteMany({ chatId, autoGenerated: true });
  ...
  for (const vaccine of VACCINATION_SCHEDULE) {
    ...
    // Chỉ tạo lịch cho vaccine chưa tiêm (ngày tiêm >= hôm nay - 7 ngày)
    if (vaccineDate.isAfter(today.subtract(7, 'day'))) {
      schedules.push({
        chatId,
